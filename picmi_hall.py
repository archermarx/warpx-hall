...
N_inject = (
    2 * dt * Lz * S0 * dx_iz / (weight * np.pi)
)  # macroparticles to inject each timestep
...
external_field = picmi.AnalyticAppliedField(
    Bx_expression="0.0", By_expression=B_func, Bz_expression="0.0"
)

# Initial particle setup
particle_layout = picmi.PseudoRandomLayout(n_macroparticles_per_cell=Np, grid=grid)
dist_e = picmi.UniformDistribution(
    n0,
    rms_velocity=[ve_std, ve_std, ve_std],
    lower_bound=[0.0, 0.0, 0.0],
    upper_bound=[Lx, Lz, Lz],
)
dist_i = picmi.UniformDistribution(
    n0,
    rms_velocity=[vi_std, vi_std, vi_std],
    lower_bound=[0.0, 0.0, 0.0],
    upper_bound=[Lx, Lz, Lz],
)
...
collision_ee = picmi.CoulombCollisions(
    "ee", species=[electrons, electrons], ndt=collision_interval
)
collision_ei = picmi.CoulombCollisions(
    "ei", species=[electrons, ions], ndt=collision_interval
)
collision_ii = picmi.CoulombCollisions(
    "ii", species=[ions, ions], ndt=collision_interval
)
...
def inject_particles():
    ...
    xs = xm_iz + np.arcsin(2 * r1 - 1) * (
        dx_iz / np.pi
    )  # x-position determined by prescribed profile
    ...
    ion_wrapper.add_particles(
        x=xs, y=ys, z=zs, ux=vel_ion[:, 0], uy=vel_ion[:, 1], uz=vel_ion[:, 2], w=ws
    )
...
def inject_cathode():
    ...
    if N_cath > 0:
        ...
        z = np.random.uniform(
            0, Lz, N_cath
        )  # Uniform distribution in azimuthal dimension
